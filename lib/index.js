...
class SQSProcessor {
    ...
    async postToWorker (messageBody, sqsMessage) {
        ...
        for(let name in sqsMessage.MessageAttributes) {
            ...
            if (name === '_datadog') {
                Object.assign(headers, JSON.parse(value.StringValue));
            } else {
    ...
    async handleMessage (sqsMessage) {
        ...
        try {
            ...
            debug("Message successfully removed from sqs, MessageId: %s ", sqsMessage.MessageId);
        } catch (err) {
            error("Error while Message process: " + err.message);

            const timeout = Math.ceil(Number(this.options.errorTimeout) / 1000); // seconds
            debug("changing failed message visibility: timeout=%d", timeout);
            await this._queue.changeMessageVisibility({
                ReceiptHandle: sqsMessage.ReceiptHandle,
                QueueUrl: this.options.queueUrl,
                VisibilityTimeout: timeout
            }).promise();

            debug("Message returned to sqs, MessageId: %s ", sqsMessage.MessageId);
        } finally {
            debug("Message stats: %o ", {MessageId: sqsMessage.MessageId, taskTime: new Date().getTime() - startTime});
    ...
    async tick  () {
        ...
        const timeout = Math.ceil(Number(this.options.timeout) / 1000 + 1); // seconds
        debug("Start Polling For %s Messages w/ VisibilityTimeout=%d", (this.options.maxMessages - this.processingMesages.length), timeout)
        this.polling = true;
        ...
        if ( data && Array.isArray(data.Messages) && data.Messages.length >0 ) {
            ...
            data.Messages.forEach( m => {
                this.processingMesages.push(m);
                m.promise = this.handleMessage(m);
            })
        }
        else
